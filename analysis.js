...
function analyze(raw) {
  ...
  const series = allMonths.map((mk) => ({
    ...
    conversion: matchesByMonth[mk] ? Math.round((reachedByMonth[mk] || 0) / matchesByMonth[mk] * 100) : 0,
  }));
  ...
  // ---- activity: sessions, active days, day×hour heatmap ----
  // Gather every message timestamp across all convos.
  const allStamps = [];
  for (const m of convos) for (const msg of sortedMessages(m)) {
    const d = new Date(msg.timestamp);
    if (!isNaN(d)) allStamps.push(d);
  }
  allStamps.sort((a, b) => a - b);

  // active days
  const dayset = new Set(allStamps.map((d) => d.toISOString().slice(0, 10)));
  const activeDays = dayset.size;

  // sessions: group consecutive stamps with gap < 30 min; sum durations
  const SESSION_GAP = 30 * 60000;
  let sessionMs = 0, sessionStart = null, prev = null, sessionCount = 0;
  for (const d of allStamps) {
    if (prev == null) { sessionStart = d; sessionCount = 1; }
    else if (d - prev < SESSION_GAP) { /* same session */ }
    else { sessionMs += (prev - sessionStart); sessionStart = d; sessionCount++; }
    prev = d;
  }
  if (sessionStart != null && prev != null) sessionMs += (prev - sessionStart);
  const sessionHours = Math.round(sessionMs / 3600000 * 10) / 10;

  // day-of-week (0=Sun) x hour heatmap
  const heat = Array.from({ length: 7 }, () => new Array(24).fill(0));
  const dowTotals = new Array(7).fill(0);
  const hourTotals = new Array(24).fill(0);
  for (const d of allStamps) {
    const dow = d.getUTCDay(), h = d.getUTCHours();
    heat[dow][h]++; dowTotals[dow]++; hourTotals[h]++;
  }
  let peakDow = 0, peakHour = 0, peakVal = -1;
  for (let dw = 0; dw < 7; dw++) for (let h = 0; h < 24; h++) if (heat[dw][h] > peakVal) { peakVal = heat[dw][h]; peakDow = dw; peakHour = h; }
  const peakHourOnly = hourTotals.indexOf(Math.max(...hourTotals));
  const peakDowOnly = dowTotals.indexOf(Math.max(...dowTotals));

  // ---- thread cards (anonymized): the deepest conversations, with metadata ----
  function threadMeta(m, idx) {
    const msgs = sortedMessages(m);
    const matchTs = matchTimestamp(m);
    let planTs = null;
    for (const x of msgs) if (reachedPlan(x.body)) { planTs = x.timestamp; break; }
    const met = (m.we_met || []).some((w) => w.did_meet_subject === "Yes");
    return {
      id: idx + 1,
      messages: msgs.length,
      words: msgs.reduce((a, z) => a + (z.body.split(/\s+/).length), 0),
      matchDate: matchTs ? matchTs.slice(0, 10) : null,
      exchangeDate: planTs ? planTs.slice(0, 10) : null,
      met,
      spanDays: (matchTs && msgs.length) ? Math.max(0, Math.round((new Date(msgs[msgs.length - 1].timestamp) - new Date(msgs[0].timestamp)) / 86400000)) : null,
    };
  }
  const rankedConvos = convos
    .map((m, i) => ({ m, meta: threadMeta(m, i) }))
    .sort((a, b) => b.meta.messages - a.meta.messages);
  const deepest = rankedConvos[0] ? rankedConvos[0].meta : null;
  const topThreads = rankedConvos.slice(0, 6).map((x) => x.meta);
  ...
  return {
    funnel: {
      ...
      reached,
      noConvo: matches.length - convos.length,
      pctMatch: likes ? Math.round(matches.length / (likes + matches.length) * 100) : 0,
      pctConvo: matches.length ? Math.round(convos.length / matches.length * 100) : 0,
      pctReached: matches.length ? Math.round(reached / matches.length * 100) : 0,
    },
    series,
    peakMatches,
    peakFlow,
    ...
    activity: {
      activeDays,
      totalMessages: allStamps.length,
      msgsPerActiveDay: activeDays ? Math.round(allStamps.length / activeDays * 10) / 10 : 0,
      sessionHours,
      sessionCount,
      heat, dowTotals, hourTotals,
      peakDow, peakHour, peakVal, peakHourOnly, peakDowOnly,
    },
    threads: { deepest, topThreads },
...
function computeContrast(matches) {
  ...
  // anonymized example shapes (clicked), sorted by length desc, top 3 — with metadata
  const examples = clicked.map((m, idx) => {
    const ms = sortedMessages(m);
    const matchTs = (m.match || [])[0] && (m.match || [])[0].timestamp;
    let planTs = null;
    for (const x of ms) if (reachedPlan(x.body)) { planTs = x.timestamp; break; }
